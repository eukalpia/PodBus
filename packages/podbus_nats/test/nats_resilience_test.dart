...
void main() {
  group('NATS JetStream resilience', () {
    ...
    test(
      'heartbeat surfaces failures when no error hook is supplied',
      () async {
        final message = _Message('jobs.long', {'id': 1})
          ..heartbeatError = const MessagingConnectionException(
            'connection lost',
          );
        final context = _JobContext(message);
    ...
    test('resilient queue recreates JetStream and restores workers', () async {
      ...
      adapters.first.publishError = const MessagingConnectionException(
        'connection lost',
      );
